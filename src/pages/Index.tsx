...
const Index = () => {
  ...
  const getConnectorPosition = (cardId: string, connector: string) => {
    ...
    // Offset for connector position based on card size (320px width, larger with padding)
    const cardWidth = 320;
    const cardHeight = card.type === 'card' ? 180 : 320;
  ...
  const checkConnectorHover = useCallback((handX: number, handY: number): string | null => {
    ...
    for (const obj of objects) {
      ...
      for (const conn of connectors) {
        ...
        switch (conn) {
          case 'left':
            connX = cardX - 16; // Approximate offset for larger card width
            break;
          case 'right':
            connX = cardX + 16;
            break;
          case 'top':
            connY = cardY - 9; // Approximate offset for card height
            break;
          case 'bottom':
            connY = cardY + 9;
            break;
